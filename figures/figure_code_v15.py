...
gs = GridSpec(3,4,figure=f,wspace=0.6,hspace=0.5)

f.text(0.001,0.95,'A',fontsize=letterLabelSize,fontname='roboto')
f.add_artist(mpatches.Rectangle((.04,.73),.69,.26,facecolor='k',zorder=0,alpha=1))
for i in range(len(imgs)):
    ax = f.add_axes([.135*i-0.06,.71,.25,.25])
    ax.imshow(imgs[i],vmin=-2,vmax=9)
    ax.axis('off')
    ax.set_title('gen %i' % (ts[i]/1000),color='white',fontsize=tickFontSize,pad=0)

cbar_ax = f.add_axes([0.7,.75,.01,.23])
...
f.text(0.73,0.95,'B',fontsize=letterLabelSize,fontname='roboto')
ax = f.add_subplot(gs[0:1,3:4])
ax.scatter(radii,np.mean(relatedness,axis=0),color='k')
ax.errorbar(radii,np.mean(relatedness,axis=0),yerr=np.std(relatedness,axis=0),fmt='None',ecolor='k')
ax.set_xlabel('Neighborhood Size\n(cell lengths)',fontsize=axisFontSize,labelpad=-2)
ax.set_ylabel('Relatedness\n(Ave. Cousin #)',fontsize=axisFontSize)
ax.set_xlim([0.8,9.2])
ax.set_xticks(np.linspace(1,9,5))
ax.set_xticks(np.linspace(1,9,9),[],minor=1)
ax.set_ylim([0,8])
ax.set_yticks(np.linspace(0,8,5))
ax.set_yticks(np.linspace(0,8,9),[],minor=1)
ax.spines['left'].set_linewidth(tickWidth)
ax.spines['bottom'].set_linewidth(tickWidth)
ax.spines['right'].set_linewidth(0)
ax.spines['top'].set_linewidth(0)
...
f.text(0.001,0.62,'C',fontsize=letterLabelSize,fontname='roboto')
ax = f.add_axes([0.02,-.15,.14,.8],anchor='NW')
ax.imshow(satpathway)
ax.axis('off')

f.text(0.17,0.62,'D',fontsize=letterLabelSize,fontname='roboto',color='white')
f.add_artist(mpatches.Rectangle((.17,.03),.56,.635,facecolor='k',zorder=0,alpha=1))
cbar_ax = f.add_axes([0.7,.05,.01,.18],zorder=1)
...
for i in range(len(molAImgs)):
    ax = f.add_axes([.12*i+0.14,.43,.2,.2])
    ...
    ax = f.add_axes([.12*i+0.14,.23,.2,.2])
    ...
    ax = f.add_axes([.12*i+0.14,.03,.2,.2])
    ...
    ax.axis('off')
ax = f.add_axes([0.18,.48,.1,.1],anchor='NW')
ax.imshow(A)
ax.axis('off')
ax = f.add_axes([0.18,.31,.1,.05],anchor='NW')
ax.imshow(B)
ax.axis('off')
ax = f.add_axes([0.18,.11,.1,.05],anchor='NW')
ax.imshow(C)
ax.axis('off')

f.text(0.73,0.62,'E',fontsize=letterLabelSize,fontname='roboto')
...
f.text(0.73,0.3,'F',fontsize=letterLabelSize,fontname='roboto')
