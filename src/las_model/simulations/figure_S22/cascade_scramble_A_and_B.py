...
for i in range(nCells):
    ...
    # Inherit A and B from different mother cell 
    sis1state_new = sis1state
    newMother_1 = rng.integers(0,nCells)
    newA_B = rng.binomial(divStates[0:2,newMother_1].astype(int),0.5)
    sis1state_new[0:2] = newA_B

    sis2state_new = sis2state
    newMother_2 = rng.integers(0,nCells)
    newA_B2 = rng.binomial(divStates[0:2,newMother_2].astype(int),0.5)
    sis2state_new[0:2] = newA_B2
...
# save to file
with open(PROJECT_DIR / 'cascade_scramble/cascade_scramble_A_and_B.pickle','wb') as f:
...
with open(PROJECT_DIR / 'cascade_scramble/cascade_scramble_A_and_B_plot.pickle','wb') as f:
    pickle.dump([vardrnd_inherited,vardsis_inherited,vardrnd_scrambled,vardsis_scrambled,normvar_inherited,normvar_scrambled],f,pickle.HIGHEST_PROTOCOL)
